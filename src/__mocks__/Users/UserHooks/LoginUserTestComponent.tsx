...
const UserTestHooks = (): JSX.Element => {
	const { formFields, loggedInUser, userError } = useUserStateContext()
	const { editFormFields } = useEditUser()
	const { loginUser, setUserError } = useGetUser()

	return (
		<div>
			<span>Login error: {userError}</span>
			...
			<button onClick={() => setUserError('New Login Error')}>Set Login Error</button>
