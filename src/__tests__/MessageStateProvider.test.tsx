...
describe('testing the message state provider', () => {
	...
	test('Message errors show up when thrown', async () => {
		render(<MessageTestApp />)

		expect(screen.getByText(/Errors/i).textContent).toBe('Errors: ')

		const errorButton = screen.getByText(/Set Message Error/i)
		fireEvent.click(errorButton)

		expect(screen.getByText(/Errors/i).textContent).toBe('Errors: New Message Error')
	})
