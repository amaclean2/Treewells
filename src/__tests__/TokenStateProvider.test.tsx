...
describe('testing the token state provider', () => {
	...
	test('Setting an error is shown', async () => {
		await renderApp()

		expect(screen.getByText(/Token error view/i).textContent).toBe('Token error view: ')
		const throwErrorButton = screen.getByText(/Set Token Error/i)
		fireEvent.click(throwErrorButton)

		expect(screen.getByText(/Token error view/i).textContent).toBe(
			'Token error view: New Token Error'
		)
	})
